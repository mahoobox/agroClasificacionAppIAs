...
class ImageClassificationHelper {
  ...
  static const modelPath =
      'assets/models/modelo_bovino_clasificacion_3_uint8.tflite';
  //static const labelsPath = 'assets/models/labels.txt';
  static const labelsPath = 'assets/models/labels_modelo.txt';
  ...
  Future<Map<String, double>> inferenceImages(List<img.Image> images) async {
    Map<String, double> combinedResults = {};
    for (var image in images) {
      // Convierte img.Image a un formato adecuado si es necesario
      var result = await inferenceImage(image);
      result.forEach((key, value) {
        combinedResults[key] = (combinedResults[key] ?? 0) + value;
      });
    }
    combinedResults.forEach((key, value) {
      combinedResults[key] = value / images.length;
    });
